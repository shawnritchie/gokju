...
func TestAllFunctionType(t *testing.T) {
	var routingContext DefaultContainerContext = NewContainerContext(containeKeyDef)
	fxTypes := routingContext.AllFunctionType()
	if (len(fxTypes) != 14) {
		t.Errorf("The amount of function defintions expected was 14 while you generated %v", len(fxTypes))
	}
}